...
updated_at: "2022-10-27 00:59:45.493514"
...
```bash
$ singularity exec <container> /usr/local/bin/ct-ng.config
$ podman run --it --rm --entrypoint /usr/local/bin/ct-ng.config   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/ct-ng.config   -v ${PWD} -w ${PWD} <container> -c " $@"
```
