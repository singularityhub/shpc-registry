...
updated_at: "2022-10-27 01:05:27.704484"
...
```bash
$ singularity exec <container> /usr/local/bin/.d
$ podman run --it --rm --entrypoint /usr/local/bin/.d   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.d   -v ${PWD} -w ${PWD} <container> -c " $@"
```
