...
updated_at: "2022-10-27 00:53:48.869335"
latest: "3.5.3--2"
...
```bash
$ singularity exec <container> /usr/local/bin/.metawatt-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.metawatt-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.metawatt-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
