...
updated_at: "2022-11-03 00:57:35.238336"
...
```bash
$ singularity exec <container> /usr/local/bin/.krona-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.krona-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.krona-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
