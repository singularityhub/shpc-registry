...
updated_at: "2022-11-21 13:02:09.249816"
...
```bash
$ singularity exec <container> /usr/local/bin/.pbaa-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.pbaa-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.pbaa-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
