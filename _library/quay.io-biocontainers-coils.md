...
updated_at: "2022-11-03 01:06:20.042254"
...
```bash
$ singularity exec <container> /usr/local/bin/.coils-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.coils-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.coils-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
