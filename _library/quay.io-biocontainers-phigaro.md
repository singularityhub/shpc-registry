...
updated_at: "2022-11-10 00:28:12.718316"
...
```bash
$ singularity exec <container> /usr/local/bin/.phigaro-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.phigaro-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.phigaro-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
