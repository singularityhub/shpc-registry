...
updated_at: "2022-11-20 00:48:22.051276"
...
```bash
$ singularity exec <container> /usr/local/bin/.mapseq-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.mapseq-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.mapseq-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
