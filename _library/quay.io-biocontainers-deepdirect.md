---
layout: container
name:  "quay.io/biocontainers/deepdirect"
maintainer: "@vsoch"
...
updated_at: "2024-05-19 03:15:59.971116"
...
```bash
$ shpc install quay.io/biocontainers/deepdirect
```
...
#### deepdirect-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### deepdirect-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/deepdirect
$ podman run --it --rm --entrypoint /usr/local/bin/deepdirect   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/deepdirect   -v ${PWD} -w ${PWD} <container> -c " $@"
```
