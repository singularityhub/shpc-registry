...
updated_at: "2022-10-29 18:04:32.584033"
...
```bash
$ singularity exec <container> /usr/local/bin/.fastml-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.fastml-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.fastml-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
