...
updated_at: "2022-11-04 00:48:45.593853"
...
```bash
$ singularity exec <container> /usr/local/bin/.molpopgen-analysis-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.molpopgen-analysis-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.molpopgen-analysis-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
