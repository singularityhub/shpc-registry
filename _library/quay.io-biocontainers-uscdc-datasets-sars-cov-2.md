...
```bash
$ singularity exec <container> /usr/local/bin/.uscdc-datasets-sars-cov-2-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.uscdc-datasets-sars-cov-2-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.uscdc-datasets-sars-cov-2-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
