...
updated_at: "2022-10-27 00:47:18.878788"
...
```bash
$ singularity exec <container> /usr/local/bin/.amptk-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.amptk-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.amptk-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
