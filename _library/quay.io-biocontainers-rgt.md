...
updated_at: "2022-10-27 00:58:52.121685"
...
```bash
$ singularity exec <container> /usr/local/bin/.rgt-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.rgt-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.rgt-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
