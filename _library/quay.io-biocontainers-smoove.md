...
updated_at: "2022-10-27 00:30:33.234173"
...
```bash
$ singularity exec <container> /usr/local/bin/lumpyexpress.config
$ podman run --it --rm --entrypoint /usr/local/bin/lumpyexpress.config   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/lumpyexpress.config   -v ${PWD} -w ${PWD} <container> -c " $@"
```
