...
updated_at: "2022-11-16 00:39:12.295330"
...
```bash
$ singularity exec <container> /usr/local/bin/.gtdbtk-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.gtdbtk-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.gtdbtk-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
