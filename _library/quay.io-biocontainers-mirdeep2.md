...
updated_at: "2022-11-02 19:10:47.694217"
...
```bash
$ singularity exec <container> /usr/local/bin/Rfam_for_miRDeep.fa
$ podman run --it --rm --entrypoint /usr/local/bin/Rfam_for_miRDeep.fa   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/Rfam_for_miRDeep.fa   -v ${PWD} -w ${PWD} <container> -c " $@"
```
