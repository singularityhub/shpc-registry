...
updated_at: "2022-10-29 18:09:04.942166"
...
```bash
$ singularity exec <container> /usr/local/bin/.ggd-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.ggd-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.ggd-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
