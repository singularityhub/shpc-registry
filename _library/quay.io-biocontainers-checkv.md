...
updated_at: "2022-11-21 01:02:04.544608"
...
```bash
$ singularity exec <container> /usr/local/bin/.checkv-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.checkv-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.checkv-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
