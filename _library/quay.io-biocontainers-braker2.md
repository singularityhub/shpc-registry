...
updated_at: "2022-11-18 01:27:03.959427"
...
```bash
$ singularity exec <container> /usr/local/bin/.braker2-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.braker2-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.braker2-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
