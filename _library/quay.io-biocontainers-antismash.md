...
```bash
$ singularity exec <container> /usr/local/bin/.antismash-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.antismash-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.antismash-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
