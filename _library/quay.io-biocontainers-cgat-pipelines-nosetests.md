...
updated_at: "2022-10-27 01:13:36.011537"
...
```bash
$ singularity exec <container> /usr/local/bin/.python-drmaa-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.python-drmaa-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.python-drmaa-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
