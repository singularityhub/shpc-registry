...
updated_at: "2022-11-22 01:41:37.233541"
...
```bash
$ singularity exec <container> /usr/local/bin/.recalladapters-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.recalladapters-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.recalladapters-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
