...
updated_at: "2022-11-10 00:15:29.079819"
...
```bash
$ singularity exec <container> /usr/local/bin/.open-cravat-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.open-cravat-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.open-cravat-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
