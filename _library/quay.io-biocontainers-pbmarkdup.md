...
updated_at: "2022-11-14 00:48:59.189796"
latest: "1.0.2--0"
...
```bash
$ singularity exec <container> /usr/local/bin/.pbmarkdup-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.pbmarkdup-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.pbmarkdup-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
