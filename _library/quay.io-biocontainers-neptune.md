...
updated_at: "2022-10-27 01:02:10.827826"
latest: "1.2.5--py_3"
...
```bash
$ singularity exec <container> /usr/local/bin/.neptune-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.neptune-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.neptune-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
