---
layout: container
name:  "quay.io/biocontainers/jronn"
maintainer: "@vsoch"
...
updated_at: "2025-09-07 03:30:32.171515"
...
```bash
$ shpc install quay.io/biocontainers/jronn
```
...
#### jronn-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### jronn-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/jronn
$ podman run --it --rm --entrypoint /usr/local/bin/jronn   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/jronn   -v ${PWD} -w ${PWD} <container> -c " $@"
```
