...
updated_at: "2022-11-02 19:19:45.072351"
...
```bash
$ singularity exec <container> /usr/local/bin/.zmwfilter-post-link.sh
$ podman run --it --rm --entrypoint /usr/local/bin/.zmwfilter-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/.zmwfilter-post-link.sh   -v ${PWD} -w ${PWD} <container> -c " $@"
```
