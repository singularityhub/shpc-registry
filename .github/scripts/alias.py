...
def get_parser():
    ...
    for command in add, remove:
        ...
        command.add_argument("path", help="alias path")
...
def main():
    ...
    if not match:
        sys.exit(
            "Cannot find {args.container} module. Are you sure it exists in this registry?"
        )
...
def add_alias(module, alias, path, force=False):
    ...
    if alias in module._config and not force:
